...
def application(environ, start_response):
    ...
    if path == '/register' and un and pw:
        ...
        if user:
            ...
            start_response('200 OK', headers)
            return ['Username created succesfully'.encode()]
        ...
        if user:
            ...
            if 'factor1' in params and 'factor2' in params and 'answer' in params:
                if int(params['factor1'][0]) * int(params['factor2'][0]) == int(params['answer'][0]):
                    page += '''<p style="background-color: lightgreen">Correct, {} x {} = {}</p>
                    '''.format(params['factor1'][0], params['factor2'][0], params['answer'][0])

                    correct += 1
                    print('correct')
                else:
                    page += '''<p style="background-color: red">Wrong, {} x {} does not equal {}</p>
                    '''.format(params['factor1'][0], params['factor2'][0], params['answer'][0])
            ...
            answer = [f1*f2, random.randint(2, 100), random.randint(2, 100), random.randint(2, 100)]

            random.shuffle(answer)


            page += '''A: <a href="/account?username={}&amp;password={}&amp;factor1={}&amp;factor2={}&amp;answer={}">{}</a><br>
            ...
            D: <a href="/account?username={}&amp;password={}&amp;factor1={}&amp;factor2={}&amp;answer={}">{}</a><br>'''.format(un, pw, f1, f2, answer[0], answer[0], un, pw, f1, f2, answer[1], answer[1], un, pw, f1, f2, answer[2], answer[2], un, pw, f1, f2, answer[3], answer[3])
